...
export default function Leaderboard({ leaderboard, user, onViewProfile }: LeaderboardProps) {
      ...
      >
        {virtualizer.getVirtualItems().map((virtualRow) => {
          ...
          return (
            ...
            >
              <div 
                onClick={() => !isUser && onViewProfile?.(u)}
                className={`rounded-xl p-3 flex items-center gap-3 transition-all duration-200 hover:scale-[1.01] cursor-pointer ${isUser ? '' : 'hover:bg-gray-900/50'}`}
                style={{
                  background: isUser
